...
public class RainbowTheme extends ThemeBase {
	 ...
	 */
	protected final IBoolean ignoreDisabled;
	/**
	 * Predicate for resetting rainbow for each button rainbow.
	 */
	protected final IBoolean buttonRainbow;
	/**
	 * Integer supplier for rainbow gradient.
	 */
	protected final IntSupplier rainbowGradient;
	/**
	 * The font size.
	 */
	...
	protected void renderRainbowRect (Rectangle rect, Context context, boolean focus) {
		...
		while (currentHue<targetHue) {
			...
			int top= Math.round((currentHue-hsb[0])*rainbowGradient.getAsInt());
			int bottom= Math.round((nextHue-hsb[0])*rainbowGradient.getAsInt());
