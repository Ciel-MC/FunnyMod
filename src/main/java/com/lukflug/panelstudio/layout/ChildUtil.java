...
public class ChildUtil {
	 ...
	 * The pop-up type.
	 */
	protected final PopupTuple popupType;
		...
		switch (mode) {
			...
			case POPUP, DRAG_POPUP -> {
				...
				if (popupType.dynamicPopup)
					popup = ClosableComponent.createDynamicPopup(button, container, state, animation.get(), new RendererTuple<T>(stateClass, theme), popupType.popupSize, toggle, width);
				else
					popup = ClosableComponent.createStaticPopup(button, container, state, animation.get(), new RendererTuple<T>(stateClass, theme), popupType.popupSize, toggle, () -> width, false, "", false);
